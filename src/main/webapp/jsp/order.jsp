...
<div id="page-container">
    <div id="content-wrap">
        ...
        <c:if test="${not empty orderDates}">
            <div class="form-group">
                <label for="exampleFormControlSelect1"><fmt:message key="profile.date"/> </label>

                <form method="get" action="show_order">
                    <select name="orderDate" onchange="this.form.submit()" class="selectpicker"
                            id="exampleFormControlSelect1">
                        <option disabled selected value> -- <fmt:message key="profile.date.select"/> --</option>
                        <c:forEach var="orderDate" items="${orderDates}">
                            <option ${orderDate==selected_date?"selected":""} value="${orderDate}">${orderDate}</option>
                        </c:forEach>
        ...
        <c:if test="${not empty concertOrder or not empty theatreOrder or not empty operaOrder
        or not empty balletOrder or not empty cinemaOrder or not empty sportOrder}">
        ...
        <table class="table">
            ...
            <c:if test="${not empty concertOrder}">
                <tr>
                    <th rowspan="${concertOrder.size() + 1}" scope="rowgroup"><fmt:message
                            key="event_type.concert"/></th>
                </tr>
                <c:forEach var="concertOrder" items="${concertOrder}">
                    <tr>
                        <th scope="row"><img
                                src="${pageContext.request.contextPath}/images/events/${concertOrder.activity.imageURL}"
                                width="70"
                                height="80"></th>
                        <td scope="row">${concertOrder.activity.name}</td>
                        <td>${concertOrder.activity.description}</td>
                        <td>${concertOrder.activity.address}</td>
                        <td>
            ...
            <!--THEATRE_PERFORMANCE -->
            <c:if test="${not empty theatreOrder}">
                <tr>
                    <th rowspan="${theatreOrder.size() + 1}" scope="rowgroup"><fmt:message
                            key="event_type.theater"/></th>
                </tr>
                <c:forEach var="theatreOrder" items="${theatreOrder}">
                    <tr>
                        <th scope="row"><img
                                src="${pageContext.request.contextPath}/images/events/${theatreOrder.activity.imageURL}"
                                width="70"
                                height="80"></th>
                        <td scope="row">${theatreOrder.activity.name}</td>
                        <td>${theatreOrder.activity.description}</td>
                        <td>${theatreOrder.activity.address}</td>
                        <td>
                            <form method="post" action="update_quantity_in_order">
                                <input type="hidden" name="command" value="update_quantity_in_order">
            ...
            <!--Order-->
            <c:if test="${not empty operaOrder}">
                <tr>
                    <th rowspan="${operaOrder.size() + 1}" scope="rowgroup"><fmt:message
                            key="event_type.opera"/></th>
                </tr>
                <c:forEach var="operaOrder" items="${operaOrder}">
                    <tr>
                        <th scope="row"><img
                                src="${pageContext.request.contextPath}/images/events/${operaOrder.activity.imageURL}"
                                width="70"
                                height="80"></th>
                        <td scope="row">${operaOrder.activity.name}</td>
                        <td>${operaOrder.activity.description}</td>
                        <td>${operaOrder.activity.address}</td>
                        <td>
                            <form method="post" action="update_quantity_in_order">
                                <input type="hidden" name="command" value="update_quantity_in_order">
                                ...
                                <div class="input-group mb-3">
                                    ...
                                    <input type="number" min="1" max="999"
                                           ...
                                           class="form-control" aria-label="Sizing example input"
                                           aria-describedby="inputGroup-sizing-default">
                                    <span style="color:red;">${operaOrder}</span>
                                </div>
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane" id="add-comment">
                                <form method="post" action="comment" class="form-horizontal" id="commentForm"
                                    ...
                                    <div class="form-group">
                                        <div class="col-sm-10">
                                                <textarea minlength="1" maxlength="200" class="form-control"
                                                          name="comment"
                                                          id="addComment"
                                                          rows="5"></textarea>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        <div class="col-sm-offset-2 col-sm-10">
                                            <button class="btn btn-success btn-circle text-uppercase" type="submit"
                                                ...
                                                <fmt:message key="profile.order.comment"/>
                                            </button>
                                        </div>
                                    </div>
                                </form>
